...
def print_shape_and_attributes(layer_number, module, input, output):
    ...
    print(f"Layer {layer_number}: {module.__class__.__name__}, Input shape: {input_shape}, Kernel size: {kernel_size}, Num filters: {num_filters}, Stride: {stride}, Output shape: {output.shape}")
...
def register_hooks(module, parent_counter=""):
    ...
    if not list(module.children()):
        ...
        hook = module.register_forward_hook(lambda module, input, output, layer_number=layer_number: print_shape_and_attributes(layer_number, module, input, output))
        hooks.append(hook)
    else:
