...
def print_header(driver_version, long_format=False):
    ...
    if long_format:
        ...
        print(
            gpu_format_col1.format("GPU", "Fan", "Temp", "Perf", "Pwr:Usage/Cap"),
            end="",
        )
...
def print_gpu_info(index, long_format=False):
    ...
    if long_format:
        print(gpu_longformat_col11.format(min_number, prod_name, pers_mode), end="")
        print(gpu_longformat_col12.format(bus_id, disp_active), end="")
        print(gpu_longformat_col13.format(ecc_error))
        print(
            gpu_longformat_col21.format(
                "{}%".format(fan_speed),
                "{}C".format(temp),
                "P{}".format(perf_state),
                "{:>4}W /{:>4}W ".format(power_draw, power_lim),
            ),
            end="",
        )
        print(
            gpu_longformat_col22.format("{:>5}MiB / {:>5}MiB".format(used, total)),
            end="",
        )
        print(gpu_longformat_col23.format("{}%".format(gpu_util), mode))
        print("+" + "-" * 33 + "+" + "-" * 21 + "+" + "-" * 21 + "+")
        n += 3
    else:
        print(
            gpu_format_col1.format(
                min_number,
                "{}%".format(fan_speed),
                "{}C".format(temp),
                perf_state,
        ...
        print(gpu_format_col2.format("{:>5}MiB / {:>5}MiB".format(used, total)), end="")
